...
- 进行中：继续 M2，已还原 `StartApp.Sy(...)` 的目标类/方法/签名；已确认 `DTHelper` 是通用 OkHttp 包装器、`jxbrowser.n` 是带 `expireTime` 的状态缓存、`StartApp$5` 更像截图/状态上报任务；已确认 `AdsCallback.getAction(String)` 与 `MiJava.getAction(String)` 在 `get_current_token`、`getLoingIsToken` 两个 action 分支中，把原始 action 字符串直接传给 `StartApp.f(String)`；M1-03 原始启动验证仍等待隔离/断网环境确认。
- 下一步：继续还原 `StartApp.f(String)` 第 385 行的最终请求 URL 拼接结果，追踪 `jxbrowser.n.c()` 的刷新触发点，并继续处理 `ClawWorkspace.vv(...)`、`JLoginNew.vS(...)`。
