...
- 进行中：M4A 静态目录已实现为 v37：九产品、76 个产品特定菜单、恢复 ID 和证据来源均已进入自动测试；默认补丁已撤除 `41/aigc/C28500001/C28500002` 临时产品菜单。尚待运行时产品选择器/侧边栏截图验收。
- 下一步：进入 M4B 免操作启动，使客户端双击后直接建立本地永久授权态并显示九产品选择器；完成后在宿主机和断网环境验收九产品卡、各系统侧边栏和主界面。
