...
- 进行中：继续 M2，已还原 `StartApp.Sy(...)` 的目标类/方法/签名；已确认 `DTHelper` 是通用 OkHttp 包装器、`jxbrowser.n` 是带 `expireTime` 的状态缓存、`StartApp$5` 更像截图/状态上报任务；已确认 `AdsCallback.getAction(String)` 与 `MiJava.getAction(String)` 在参数包含 `get_current_token` 或 `getLoingIsToken` 时，把原始 URL/参数字符串直接传给 `StartApp.f(String)`；该方法追加 `RT`，`DTHelper` 再追加 `rdtime` 后交给 OkHttp；M1-03 原始启动验证仍等待隔离/断网环境确认。
- 下一步：追踪 `jxbrowser.n.c()` 的刷新触发点，并继续处理 `ClawWorkspace.vv(...)`、`JLoginNew.vS(...)`。
