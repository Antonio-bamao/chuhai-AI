...
- M5A 已启动静态分类：新增 `.context/m5a-business-dependency-inventory.md`，将业务能力分为客户端直连第三方、原后端代理、原后端数据库、云资源/算力和 native 依赖。该清单尚未执行真实业务动作，只作为后续只读验收和 M5B 重建范围的证据底稿。
...
| M5 真实业务联网回归 | 静态分类启动 | 约 30% | 已形成 M5A 第一版依赖分类；只验证了 AiCloud 在线首屏，尚未跑通真实业务动作。 |
