...
脚本实际抓到 1,001 条调用记录；其中包含同一行多个调用、以及后续匹配到的同类调用。当前可读性评分 `>= 0.8` 的记录有 943 条，第一版可作为 M3 检索输入。
...
按 `auth/license/login/token/expire/pay/order/user/tenant/role/header/result/data` 以及中文关键词筛选出 132 条候选，已写入：
...
1. 解析 `JLoginHTML$h.v(...)`、`ClawWorkspace.vv(...)` 等 j2026 相关字符串/动态调用。
