...
| M5 真实业务联网回归 | 只读验收与路由发现 | 约 52% | 已形成 M5A 第一版依赖分类、第一轮只读证据、菜单路由发现、spider/dataCollect 入口发现，并通过 v49 宿主只读验证 WhatsApp `AI采集` 恢复子路由可到达 dataCollect 空表页；页面层和只读本地结果列表已打通，但任务提交、队列、结果保存和云资源仍未执行或验收。 |
